...
class ViewFriendFullProfileController extends GetxController {
  ViewFriendFullProfileController(this.viewFriendFullProfileModelObj);

  Rx<ViewFriendFullProfileModel> viewFriendFullProfileModelObj;

  SelectionPopupModel? selectedDropDownValue;

  onSelected(dynamic value) {
    for (var element
        in viewFriendFullProfileModelObj.value.dropdownItemList.value) {
      element.isSelected = false;
      if (element.id == value.id) {
        element.isSelected = true;
      }
    }
    viewFriendFullProfileModelObj.value.dropdownItemList.refresh();
  }
}
